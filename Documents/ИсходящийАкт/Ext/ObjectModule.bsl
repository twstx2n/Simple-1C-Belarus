...
#Область СебестоимостьФИФО
	...
	Если Товары.Количество() > 0 И ЮрЛицо.УчетСтоимости = Перечисления.УчетСтоимости.ФИФО Тогда 
		
		// Начинаем смотреть хватает ли товара, 
		// делаем запрос чтобы сформировать отгружаемый товар
		Запрос = Новый Запрос;
		...
		|ИЗ
		|  РегистрНакопления.ОстаткиТоваров.Остатки(&ВремяОстатков, Склад = &Склад И ЮрЛицо = &ЮрЛицо
		|    И Номенклатура В
		|      (ВЫБРАТЬ 
		|		  ДокТЧ.Номенклатура
		|       ИЗ
		...
		|ПОМЕСТИТЬ Остатки
		|ИЗ
		|	РегистрНакопления.СтоимостьТоваров.Остатки(&ВремяСтоимостиТоваров, ЮрЛицо = &ЮрЛицо И
		|			Номенклатура В
		|				(ВЫБРАТЬ
...
#Область СебестоимостьПоСтоимости
	Если Товары.Количество() > 0 И ЮрЛицо.УчетСтоимости = Перечисления.УчетСтоимости.Себестоимость Тогда
...
#Область СебестоимостьПоСреднему
	Если Товары.Количество() > 0 И ЮрЛицо.УчетСтоимости = Перечисления.УчетСтоимости.Средневзвешенное Тогда
		...
		Пока ВыборкаТовар.Следующий() Цикл
			...
			Себестоимость = ВыборкаТовар.Количество / ВыборкаТовар.КоличествоОстаток * ВыборкаТовар.СтоимостьОстаток;
			Движение = Движения.СтоимостьТоваров.ДобавитьРасход();
			Движение.Период = Дата;
			Движение.ЮрЛицо = ЮрЛицо;
			Движение.Номенклатура = ВыборкаТовар.Номенклатура;
			...
			Проводка.ЮрЛицо = ЮрЛицо;
			Проводка.СчетДт = ПланыСчетов.ПланСчетов.С90с4;
			Проводка.СчетКт = ВыборкаТовар.СчетУчета;
			Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.Номенклатура] = ВыборкаТовар.Номенклатура;
			Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.Склады] = Склад;
			Проводка.КоличествоКт = ВыборкаТовар.Количество;
			Проводка.Сумма = Себестоимость;
